...
| [`CMA_coordinate_specialist_team.ipynb`](CMA_coordinate_specialist_team.ipynb) | Heterogeneous multiagent via `callable_agents`: a coordinator runs three specialists (web-search researcher, file-reading librarian, rules-based pricer) with scoped toolsets to assemble a sales proposal. Covers the `callable_agents` field, the `thread_created` / `thread_message_received` event types, and why per-role tool scoping matters. |
| [`CMA_verify_with_outcome_grader.ipynb`](CMA_verify_with_outcome_grader.ipynb) | Iterative grader loop via `define_outcome`: a writer drafts a cited research brief, a stateless grader independently fetches every URL and checks every quote against a rubric, and feedback drives revisions until satisfied. Covers `user.define_outcome`, the `span.outcome_evaluation_*` events, and writing rubrics that verify rather than just instruct. |
