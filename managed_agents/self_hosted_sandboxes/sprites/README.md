...
Deploy either anywhere that can reach the Sprites API (Fly, Render, a VM behind a tunnel, etc.); for the webhook, register its URL as the webhook endpoint — `POST /v1/webhooks` with `{"url": ..., "enabled_events": ["session.status_run_started"]}` returns the `signing_secret` to export as `ANTHROPIC_WEBHOOK_SECRET`.

A Sprite itself makes a good webhook host: run uvicorn as the Sprite's service with `http_port`, and set the URL public (`PUT /v1/sprites/{name}` with `{"url_settings": {"auth": "public"}}`) since deliveries carry no credentials — the signature is the auth, and every unsigned request gets a 401. Two properties fall out of the pause model: an inbound delivery wakes the orchestrator Sprite if it has paused, so it needs no keep-alive of its own; and because `spawn()` runs inside the handler (~15–20s on a cold Sprite), the response can outlive the sender's delivery timeout, so expect an occasional duplicate delivery in the log — harmless, since the retry's drain finds the work item already leased and does nothing.
