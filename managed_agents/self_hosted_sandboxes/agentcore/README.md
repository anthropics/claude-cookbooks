...
- `tui_shell.py` — operator TUI, interactive PTY over WebSocket
  (`InvokeAgentRuntimeCommandWithWebSocketStream`). Local terminal
  attaches to a real bash PTY in the microVM with line editing, Ctrl+C,
  and `SIGWINCH` resize.
...
- An AWS account with [AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html)
  ...
  `bedrock-agentcore:InvokeAgentRuntime` on the deployed runtime ARN).
...
Interactive PTY over WebSocket — full bash session with line editing,
Ctrl+C, and resize:
